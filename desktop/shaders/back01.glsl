...
{
    vec2 r = vec2(p.x + sin(time*speed / 2.0 * fx) * ax / speed, p.y + cos(time*speed / 2.0 * fy) * ay / speed);
    return .027 / length(r / sin(fy * time * 0.27));
}
...
void main(void)
{
    ...
    gl_FragColor = vec4(
                        pow(col * 0.27 * col2, 3.0 / col),
                        col * 0.34,
                        col2 * 0.9 * cos(time), 1.0) + vec4(col2 * 0.33,
                                                            col * col2 * 0.24 * cos(time),
                                                            col2 * 0.9,
                                                            1.0);
}
